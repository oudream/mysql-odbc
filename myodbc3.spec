...
##############################################################################
#
# mysql-connector-odbc 8.0 RPM specification
#
##############################################################################
...
Summary:	An ODBC 8.0 driver for MySQL - driver package
...
Source0:	http://cdn.mysql.com/Downloads/Connector-ODBC/8.0/%{name}-8.0.11-src.tar.gz
...
%if 0%{?odbc_gui}
%package setup
Summary:	An ODBC 8.0 driver for MySQL - setup library
Group:		Application/Databases
Requires:	%{name} = %{version}-%{release}
%endif

%package test
Summary:	An ODBC 8.0 driver for MySQL - tests
Group:		Application/Databases
Requires:	%{name} = %{version}-%{release}
...
if [ -x /usr/bin/myodbc-installer ]; then
    /usr/bin/myodbc-installer -a -d -n "MySQL ODBC 8.0 Unicode Driver" -t "DRIVER=%{_libdir}/libmyodbc8w.so"
    /usr/bin/myodbc-installer -a -d -n "MySQL ODBC 8.0 ANSI Driver"    -t "DRIVER=%{_libdir}/libmyodbc8a.so"
fi

%if 0%{?odbc_gui}
%post setup
/usr/bin/myodbc-installer -r -d -n "MySQL ODBC 8.0 Unicode Driver"
/usr/bin/myodbc-installer -r -d -n "MySQL ODBC 8.0 ANSI Driver"
/usr/bin/myodbc-installer -a -d -n "MySQL ODBC 8.0 Unicode Driver" -t "DRIVER=%{_libdir}/libmyodbc8w.so;SETUP=%{_libdir}/libmyodbc8S.so"
/usr/bin/myodbc-installer -a -d -n "MySQL ODBC 8.0 ANSI Driver"    -t "DRIVER=%{_libdir}/libmyodbc8a.so;SETUP=%{_libdir}/libmyodbc8S.so"
%endif
...
%preun
myodbc-installer -r -d -n "MySQL ODBC 8.0 Unicode Driver"
myodbc-installer -r -d -n "MySQL ODBC 8.0 ANSI Driver"
...
if [ "$1" = 0 ]; then
    if [ -x %{_bindir}/myodbc-installer ]; then
        %{_bindir}/myodbc-installer -r -d -n "MySQL ODBC 8.0 Unicode Driver" > /dev/null 2>&1 || :
        %{_bindir}/myodbc-installer -r -d -n "MySQL ODBC 8.0 ANSI Driver"    > /dev/null 2>&1 || :
        %{_bindir}/myodbc-installer -a -d -n "MySQL ODBC 8.0 Unicode Driver" -t "DRIVER=%{_libdir}/libmyodbc8w.so" > /dev/null 2>&1 || :
        %{_bindir}/myodbc-installer -a -d -n "MySQL ODBC 8.0 ANSI Driver"    -t "DRIVER=%{_libdir}/libmyodbc8a.so" > /dev/null 2>&1 || :
    fi
fi
%endif
